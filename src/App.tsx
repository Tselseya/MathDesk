...
export default function App() {
  ...
  if (workspaceOpen) return <main className="app-shell"><nav className="topbar"><button className="back-link" onClick={() => setWorkspaceOpen(false)}><ArrowLeft size={17} /> Home</button><a className="brand" href="/" aria-label="MathDesk home"><span className="brand-mark">∑</span><span>Math<span>Desk</span></span></a><div className="topbar-actions"><span className="branch-pill">redesign / phase 3</span><AuthPanel compact /></div></nav><ChatWorkspace initialMode={mode} initialPrompt={prompt} /></main>;

  return <main className="shell">
    <nav className="topbar"><a className="brand" href="/" aria-label="MathDesk home"><span className="brand-mark">∑</span><span>Math<span>Desk</span></span></a><div className="topbar-actions"><span className="branch-pill">redesign / phase 3</span><a className="icon-link" href="https://github.com/Tselseya/MathDesk" target="_blank" rel="noreferrer" aria-label="Open MathDesk on GitHub"><GitBranch size={18} /></a><AuthPanel compact /></div></nav>
